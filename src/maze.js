...
export default class Maze {
  ...
  constructor(height, width, wallColor, pathColor, wallSize, pathSize, type, shadowColor, shadowBlur, goalColor, backgroundRects = { num: 0, border: "#FFF", fill: "#FFF" }, effects) {
    ...
    else if (this.type === "maze3") { 
      this.maze = Maze.generateMazeSidewinder(new Grid(this.height, this.width)); 
      this.maze.getCell(0, 0).left = false; // creates entrance
      this.maze.getCell(this.maze.height - 1, this.maze.width - 1).right = false; // creates exit
    }
  ...
  /** 
   * Generating a maze with Sidewinder algorithm
   * @param {Grid} grid
   * @returns {Grid} a grid with a generated maze
   */
  static generateMazeSidewinder(grid) {
    let currRow, currCol, startRow, startCol, visited, potDirs;
    potDirs = [Dirs.LEFT, Dirs.RIGHT, Dirs.UP, Dirs.DOWN];
    startRow = Math.floor(Math.random() * grid.height);
    startCol = Math.floor(Math.random() * grid.width);
    ...
    const maze = this.grid;
      ...
      for (let currCol = 0; currCol < startRow; currCol++) {
        runStart = 0;
        for (let currRow = 0; currRow < startCol; currRow++){
          if (currCol > 0 && (currRow + 1 === grid.startCol || Math.random() <= .5)) {
            cell = runstart + Math.floor(Math.random() * (currRow - runStart + 1));
            maze.cell[currCol][cell].removeWall(maze.cells[currCol - 1]);
            runStart = currRow + 1; 
          }
          else if (currRow + 1 < startCol) {
            maze.cells[currCol][currRow].removeWall(maze.cells[currCol][currRow + 1]);
        }
      }
      maze.cells[currCol][currRow].visited = true;
      }
      return grid; 
  }
  /**
    ...
    for (let i = 0; i < this.maze.height; i++) {
      for (let j = 0; j < this.maze.width; j++) {
        ...
        if (this.effects === true) {
          if (cell.left) {
            context.moveTo(x, y);
            if (this.type === "maze"|| this.type === "maze2" || this.type === "maze3") {
